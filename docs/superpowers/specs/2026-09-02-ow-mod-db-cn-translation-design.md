...
- **设计为官方站点的"翻译镜像"**: 参照 outerwildsmods.com 的布局与配色(深色底 `#0b0d10`、强调色 `#ff9c86`、卡片网格、首页"热门/热门新/最近更新"三栏、全部 MOD 列表页、详情页)
- 官方仓库 `ow-mods/outerwildsmods.com` 公开可读但**无 License** → 仅参考其设计,不复制其代码,全部自行实现
...
- 术语表随团队翻译经验持续扩充
- 网站可后续加"翻译贡献"入口
- **术语表覆盖检测**(用户 2026-09-02 确定): 扫描 mod 文本,找出术语表/翻译未覆盖的专有名词。匹配策略:**不做整表线性扫描,采用两段式匹配 —— 先按词条首字母分桶筛出候选,再对候选做精确匹配**(例如小写/大写/换行分隔变体需另行归一化处理)
